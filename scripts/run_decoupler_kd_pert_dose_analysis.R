...
temp_meta = args[8]
temp_netw = args[9]
pert_time = as.double(args[10])
pert_dose = as.double(args[11])
...
sub_control$target <- off_targ

n_size = pmin(dim(sub_control)[[1]],dim(sub_meta)[[1]])
joined_meta <- bind_rows(sample_n(sub_meta,size=n_size),sample_n(sub_control,size=n_size))
print(dim(joined_meta))
