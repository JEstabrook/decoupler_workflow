...
set.seed(42)
n_sample <- floor(nrow(netw) * as.numeric(expand_perc))

random_targets <- sample(edges, n_sample, replace = TRUE)
random_regs <- sample(edges, n_sample, replace = TRUE)
...
expanded_netw <- bind_rows(netw, random_edges)
expanded_netw <- expanded_netw[order(expanded_netw$tf, expanded_netw$tf),]

# Save 
saveRDS(expanded_netw, netw_noise_out)

# Identify secondary edges for Priori
# Re-format primary interaction tibble
primary_int <- expanded_netw %>% 
